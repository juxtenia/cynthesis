...
struct state addkey(struct state in, struct state key);

// Cipher is the main function that encrypts the PlainText.
inline struct state addkey(struct state in, struct state key)
{
  uint8_t round = 0;
...
static void test_encrypt_ecb_verbose(void)
{
    ...
    for(i = 0; i < 4; ++i)
    {
        printstate("", addkey(plain_text[i], key));
    }
    printf("\n");
}
...
void test_encrypt_ecb(void)
{
  ...
  struct state out = {0x40, 0xbf, 0xab, 0xf4, 0x06, 0xee, 0x4d, 0x30, 0x42, 0xca, 0x6b, 0x99, 0x7a, 0x5c, 0x58, 0x16};

  struct state result = addkey(in, key);
