...
struct state newkey(struct state in, struct state key);

// Cipher is the main function that encrypts the PlainText.
inline struct state newkey(struct state in, struct state key)
{
  uint8_t round = 1;
  ...
  //Add round key
  struct state t7 = statestatexor(in, key);
  return t7;
}
...
static void test_encrypt_ecb_verbose(void)
{
    ...
    for(i = 0; i < 4; ++i)
    {
        printstate("", newkey(plain_text[i], key));
    }
    printf("\n");
}
...
void test_encrypt_ecb(void)
{
  ...
  struct state out = {0xcb, 0x3b, 0x40, 0xf5, 0xa6, 0x14, 0xb3, 0x27, 0xca, 0x9e, 0x47, 0x28, 0x59, 0xff, 0x61, 0x2f};

  struct state result = newkey(in, key);
