...
let weightedtimecost (f:funmodule) = 
	f.vmodules <- List.sort (fun m1 m2 -> m1.mid - m2.mid) f.vmodules;
	let ids = List.map (fun m -> m.mid) f.vmodules
